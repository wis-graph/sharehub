import { fetchFile } from '../services/github.js';
import { renderMarkdown } from '../services/renderer.js';

export async function renderNoteViewer(container, path, config) {
  container.innerHTML = `
  ...
  try {
    const { content } = await fetchFile(path, config.github);
      ...
      container.innerHTML = `
        <div class="note-content mx-auto px-6 py-8">
          <div class="text-center py-12">
            ...
            <p class="text-gray-600">The note (${path}) does not exist.</p>
          </div>
        </div>
      `;
      return;
    ...
    container.innerHTML = `
      <div class="note-content mx-auto px-6 py-8">
        <div class="markdown prose prose-slate max-w-none">
          ${await renderMarkdown(content, config.github)}
        </div>
      </div>
    `;
  } catch (error) {
    ...
    container.innerHTML = `
      <div class="note-content mx-auto px-6 py-8">
        <div class="text-center py-12">
          ...
          <h2 class="text-xl font-semibold text-gray-900 mb-2">Error loading note</h2>
